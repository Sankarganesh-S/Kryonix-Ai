...
def _user_dict(u: User):
    return {"id": u.id, "email": u.email, "username": u.username, "role": u.role, "is_verified": u.is_verified}
...
def register(req: RegisterReq, bg: BackgroundTasks, db: Session = Depends(get_db)):
    ...
    user = User(email=req.email.lower(), username=req.username, hashed_password=hash_password(req.password))
    ...
    if smtp_configured:
        ...
        return {"message": "OTP sent to your email. Please verify to continue.", "requires_otp": True, "email": user.email}
    else:
        # No SMTP configured — auto verify and login
        user.is_verified = True; db.commit()
        token = create_access_token({"sub": str(user.id)})
        return {"access_token": token, "token_type": "bearer", "user": _user_dict(user), "requires_otp": False}

# ── Step 2: Verify OTP after register
@router.post("/verify-otp")
def verify_otp_route(req: OTPReq, db: Session = Depends(get_db)):
    ok, msg = verify_otp(req.email.lower(), req.otp)
    if not ok:
...
def login(req: LoginReq, bg: BackgroundTasks, db: Session = Depends(get_db)):
    ...
    if smtp_configured:
        ...
        token = create_access_token({"sub": str(user.id)})
        user.last_seen = datetime.now(UTC); db.commit()
        return {"access_token": token, "token_type": "bearer", "user": _user_dict(user), "requires_otp": False}
...
def forgot_password(req: RequestOTPReq, bg: BackgroundTasks, db: Session = Depends(get_db)):
    user = db.query(User).filter(User.email == req.email.lower()).first()
    if not user:
...
def reset_password(req: ResetPasswordReq, db: Session = Depends(get_db)):
    ...
    db.commit()
    token = create_access_token({"sub": str(user.id)})
    return {"access_token": token, "token_type": "bearer", "user": _user_dict(user), "message": "Password reset successful"}
