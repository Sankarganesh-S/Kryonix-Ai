...
def process_file(filename: str, content_type: str, data: bytes) -> dict:
    ...
    if not file_type:
        ...
        elif ext in (".txt", ".md"): file_type = "txt"
    ...
    if file_type == "image":
        ...
        return {"type": "image", "base64": b64, "content_type": content_type, "filename": filename}

    elif file_type == "pdf":
        text = extract_text_from_pdf(data)
        return {"type": "text", "content": text[:4000], "filename": filename, "file_type": "PDF"}

    elif file_type == "docx":
        text = extract_text_from_docx(data)
        return {"type": "text", "content": text[:4000], "filename": filename, "file_type": "Word Document"}

    elif file_type == "txt":
        text = data.decode("utf-8", errors="ignore")
        return {"type": "text", "content": text[:4000], "filename": filename, "file_type": "Text File"}

    elif file_type == "csv":
        text = extract_text_from_csv(data)
        return {"type": "text", "content": text[:4000], "filename": filename, "file_type": "CSV File"}
