...
import { Copy, Check, RefreshCw, User, Sparkles, ThumbsUp, Heart, Laugh, Frown, Zap } from 'lucide-react'
import ReactMarkdown from 'react-markdown'
import { Prism as SyntaxHighlighter } from 'react-syntax-highlighter'
import { oneDark, oneLight, dracula, nord, atomDark } from 'react-syntax-highlighter/dist/esm/styles/prism'

const CODE_THEMES = { 'One Dark': oneDark, 'One Light': oneLight, 'Dracula': dracula, 'Nord': nord, 'Atom Dark': atomDark }
const REACTIONS = [
...
export default function MessageBubble({ message, onRegenerate, loading }) {
  ...
  const codeTheme = CODE_THEMES[localStorage.getItem('kryonix_code_theme') || 'One Dark'] || oneDark
    ...
    code({ node, inline, className, children, ...props }) {
      ...
      return (
        <div className="code-block">
          ...
          <SyntaxHighlighter style={codeTheme} language={lang || 'text'} PreTag="div"
            customStyle={{ margin: 0, borderRadius: '0 0 10px 10px', fontSize: 13, background: '#0d0d14' }} {...props}>
            {code}
          </SyntaxHighlighter>
        </div>
      )
    ...
    <div className={`bubble-row ${isUser ? 'user' : 'assistant'}`}
      ...
      <div className={`bubble ${isUser ? 'bubble-user' : 'bubble-assistant'} ${message.error ? 'bubble-error' : ''}`}>
        ...
        {isUser
          ...
          : <div className="markdown-body">
              {showPreview
                ? <pre style={{ whiteSpace: 'pre-wrap', fontSize: 13, color: 'var(--muted)' }}>{message.content}</pre>
        ...
        {!isUser && !message.streaming && message.content && (
          <div className="bubble-actions">
            ...
            <button onClick={() => setShowPreview(p => !p)} className="copy-btn" title="Toggle markdown preview"
      ...
      {!isUser && showReactions && !message.streaming && (
        <div className="reaction-picker">
          {REACTIONS.map(r => (
            <button key={r.label} onClick={() => setReaction(reaction === r.emoji ? null : r.emoji)}
