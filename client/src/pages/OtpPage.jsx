...
export default function OtpPage() {
  ...
  const submitOtp = async (otp) => {
    setError(null)
    const endpoint = purpose === 'login' ? '/auth/login-otp' : '/auth/verify-otp'
  ...
  const handleResend = async () => {
    ...
    if (r.ok) { setResendMsg('OTP resent! Check your email.'); setResendCooldown(60) }
  ...
  return (
    <div className="auth-bg">
      ...
      <div className="auth-card" style={{ textAlign: 'center' }}>
        ...
        <div style={{ width: 56, height: 56, borderRadius: 18, background: 'rgba(124,110,245,.12)', border: '1px solid rgba(124,110,245,.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px' }}>
        ...
        <p style={{ color: '#a78bfa', fontWeight: 600, fontSize: 14, marginBottom: 28 }}>{email}</p>

        {error && <div className="auth-alert"><AlertCircle size={15}/><span>{error}</span></div>}
        {resendMsg && <div className="auth-success" style={{ marginBottom: 16 }}><span>{resendMsg}</span></div>}
        ...
        <div style={{ marginTop: 20, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
          <span style={{ fontSize: 14, color: 'var(--muted)' }}>Didn't receive it?</span>
          <button
            onClick={handleResend}
            disabled={resendCooldown > 0}
            style={{ background: 'none', border: 'none', color: resendCooldown > 0 ? 'var(--muted)' : '#a78bfa', fontSize: 14, fontWeight: 600, cursor: resendCooldown > 0 ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}
          >
            <RefreshCw size={13}/>
